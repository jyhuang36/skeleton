...
class Skeleton(nn.Module):
    ...
    def forward(self, x):
        ...
        y2s = self.scale2(x_)[:,:,0:x.shape[2],0:x.shape[3]]

        x_ = self.conv3(x_)
        y3 = self.dsn3(x_)[:,:,0:x.shape[2],0:x.shape[3]]
        y3s = self.scale3(x_)[:,:,0:x.shape[2],0:x.shape[3]]

        x_ = self.conv4(x_)
        y4 = self.dsn4(x_)[:,:,0:x.shape[2],0:x.shape[3]]
        y4s = self.scale4(x_)[:,:,0:x.shape[2],0:x.shape[3]]

        x_ = self.conv5(x_)
        y5 = self.dsn5(x_)[:,:,0:x.shape[2],0:x.shape[3]]
        y5s = self.scale5(x_)[:,:,0:x.shape[2],0:x.shape[3]]
        ...
        return y2, y3, y4, y5, yf, y2s, y3s, y4s, y5s    
...
class SkeletonTrainingSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.threshold:
            ...
            label2s = label * (label2 > 0).astype(int) * 2 / self.threshold[1] - 1
            label3s = label * (label3 > 0).astype(int) * 2 / self.threshold[2] - 1
            label4s = label * (label4 > 0).astype(int) * 2 / self.threshold[3] - 1
            label5s = label * (label5 > 0).astype(int) * 2 / self.threshold[4] - 1

        return image, label2, label3, label4, label5, label2s, label3s, label4s, label5s
...
def main():
    ...
    optimizer = torch.optim.Adam([{'params': model.conv1.parameters()},
                                  ...
                                  {'params': model.scale2.parameters()},
                                  {'params': model.scale3.parameters()},
                                  {'params': model.scale4.parameters()},
                                  {'params': model.scale5.parameters()},
    ...
    if args.train:        
        ...
        torch.save(model.state_dict(), 'skeleton_reg.pt')
...
def loss_scale(y, label):
    ...
    label_weight.requires_grad = False
    label = label.type(dtype_float)
...
def train(model, train_loader, optimizer, scheduler): 
    ...
    for epoch in range(args.epochs):
        ...
        for i, (image, label2, label3, label4, label5, label2s, label3s, label4s, label5s) in enumerate(train_loader):
            ...
            y2, y3, y4, y5, yf, y2s, y3s, y4s, y5s = model(image)
            
            loss = (loss_class(yf, label5) + loss_class(y2, label2) + loss_class(y3, label3) + \
                    loss_class(y4, label4) + loss_class(y5, label5) + loss_scale(y2s, label2s) + \
                    loss_scale(y3s, label3s) + loss_scale(y4s, label4s) + loss_scale(y5s, label5s))/args.itersize
